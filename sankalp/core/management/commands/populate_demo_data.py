"""
Django management command to create demo data
...
class Command(BaseCommand):
    help = 'Populate database with demo data for SANKALP presentation'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('=' * 60))
        self.stdout.write(self.style.SUCCESS('CREATING DEMO DATA FOR SANKALP'))
        self.stdout.write(self.style.SUCCESS('=' * 60))
        ...
        for student in students:
            ...
            if created:
                ...
                self.stdout.write(f"   - Student {student['first_name']} already exists")
        ...
        for staff_member in staff:
            ...
            if created:
                ...
                user.save()
                self.stdout.write(f"   ✓ Created Staff: {staff_member['first_name']} {staff_member['last_name']}")
            else:
                self.stdout.write(f"   - Staff {staff_member['first_name']} already exists")
        ...
        if created:
            ...
            admin_user.save()
            self.stdout.write(f"   ✓ Created Admin: {admin['first_name']} {admin['last_name']}")
        else:
        ...
        for i, complaint in enumerate(complaint_data):
            ...
            if created:
                ...
                comp.save()
                self.stdout.write(f"   ✓ Created: {complaint['title'][:50]}... by {user.get_full_name()}")
            else:
                self.stdout.write(f"   - Complaint already exists: {complaint['title'][:50]}...")
        ...
        comment_count = 0
        for complaint in complaint_objects[:8]:
        ...
        self.stdout.write(f"   ✓ Created {comment_count} staff comments")
        ...
        self.stdout.write(f"\n✓ Users Created:")
        self.stdout.write(f"  - Students: {len(student_objects)}")
        self.stdout.write(f"  - Staff: {len(staff_objects)}")
        ...
        self.stdout.write(self.style.SUCCESS('=' * 60))
        self.stdout.write(f"\nStudent Account:")
        self.stdout.write(f"  Username: student1")
        ...
        self.stdout.write(self.style.SUCCESS('\n' + '=' * 60))
        self.stdout.write(self.style.SUCCESS('✓ Database ready for presentation!'))
        self.stdout.write(self.style.SUCCESS('=' * 60))
